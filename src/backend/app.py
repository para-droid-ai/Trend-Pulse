...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        scheduler = TopicStreamScheduler(
            SessionLocal, perform_search_and_create_summary
        )
        logger.info("TopicStreamScheduler initialized and its thread started.")
    except Exception as e:
        logger.error(
            f"Failed to initialize scheduler during startup: {e}", exc_info=True
        )
    # finally:
...
app = FastAPI(
    title="TrendPulse Dashboard API", lifespan=lifespan
)  # Ensure lifespan is used here
...
class TopicStreamResponse(BaseModel):
    ...
    class Config:
        ...
        json_encoders = {Enum: lambda v: v.value if isinstance(v, Enum) else v}
...
async def get_current_user(
    ...
    try:
        ...
        logger.error(
            f"Database error in get_current_user: {str(db_error)}", exc_info=True
        )
        raise HTTPException(
...
async def perform_search_and_create_summary(
    ...
    try:
        logger.info(
            f"Performing search for stream ID: {topic_stream.id}. Override ignore all: {ignore_all_previous_summaries_override}"
        )
        ...
        if ignore_all_previous_summaries_override:
            logger.info(
                f"Stream {topic_stream.id}: Manual override ON. Ignoring all previous summaries for this update."
            )
            ...
            if history_level_setting == ContextHistoryLevel.NONE:
                ...
                num_summaries_to_fetch = (
                    15  # Max to fetch before token-based truncation
                )

            logger.info(
                f"Stream {topic_stream.id}: Configured to include up to {num_summaries_to_fetch} (level: {history_level_setting.value}) previous summaries."
            )

        if num_summaries_to_fetch > 0:
            # Fetch summaries (content and creation date), newest first
            recent_summaries_from_db = (
                db.query(models.Summary.content, models.Summary.created_at)
                .filter(models.Summary.topic_stream_id == topic_stream.id)
                .order_by(models.Summary.created_at.desc())
                .limit(num_summaries_to_fetch)
                .all()
            )

            if recent_summaries_from_db:
                # Reverse to process oldest first for concatenation to build chronological context
                summaries_content_chronological = [
                    data.content for data in reversed(recent_summaries_from_db)
                ]
                ...
                for i, content_item in enumerate(summaries_content_chronological):
                    ...
                    effective_separator_tokens = (
                        separator_tokens if concatenated_parts else 0
                    )

                    if (
                        current_total_tokens_for_history
                        + item_tokens
                        + effective_separator_tokens
                        <= MAX_PREV_CONTEXT_TOKENS_SMART_LIMIT
                    ):
                        ...
                        current_total_tokens_for_history += (
                            item_tokens + effective_separator_tokens
                        )
                    else:
                        remaining_token_budget = MAX_PREV_CONTEXT_TOKENS_SMART_LIMIT - (
                            current_total_tokens_for_history
                            + effective_separator_tokens
                        )
                        if (
                            remaining_token_budget > 50
                        ):  # Only add if a meaningful chunk can be added
                            if concatenated_parts:
                                concatenated_parts.append(separator)
                            truncated_item_content = truncate_text_by_tokens(
                                content_item, remaining_token_budget
                            )
                            concatenated_parts.append(truncated_item_content)
                            # No need to update current_total_tokens_for_history further as we break
                            logger.info(
                                f"Stream {topic_stream.id}: Truncated content of summary part {i+1} to fit token limit."
                            )
                        else:
                            logger.info(
                                f"Stream {topic_stream.id}: Could not fit summary part {i+1} or a meaningful portion into context due to token limit."
                            )
                        break

                if concatenated_parts:
                    prev_summaries_concatenated_content = "".join(concatenated_parts)
                    final_history_tokens = count_tokens(
                        prev_summaries_concatenated_content
                    )  # Recalculate final token count precisely
                    logger.info(
                        f"Stream {topic_stream.id}: Using {len(recent_summaries_from_db)} fetched, effectively {len(concatenated_parts) // 2 + (1 if len(concatenated_parts) % 2 != 0 else 0) if separator_tokens > 0 else len(concatenated_parts)} summaries in concatenated context. Total est. tokens for history: {final_history_tokens}."
                    )
                else:
                    logger.info(
                        f"Stream {topic_stream.id}: No previous summaries fit within token limit for context."
                    )
            else:
                logger.info(
                    f"Stream {topic_stream.id}: No previous summaries found in DB to include in context."
                )
        else:
            logger.info(
                f"Stream {topic_stream.id}: Not including any previous summaries (num_summaries_to_fetch is 0 or overridden)."
            )
        ...
        if word_count_instruction:
            if final_system_prompt_for_api:
                final_system_prompt_for_api = (
                    f"{final_system_prompt_for_api.rstrip()} {word_count_instruction}"
                )
            else:
                ...
                final_system_prompt_for_api = (
                    f"{default_perplexity_system_prompt} {word_count_instruction}"
                )

        logger.debug(
            f"For stream {topic_stream.id} - Final User Query for API: {full_query[:200]}..."
        )
        # logger.debug(f"For stream {topic_stream.id} - Using Custom System Prompt: '{stream_custom_system_prompt[:100]}...'") # Old log
        if final_system_prompt_for_api:
            logger.debug(
                f"For stream {topic_stream.id} - Effective System Prompt for API: {final_system_prompt_for_api[:250]}..."
            )
        else:
            logger.debug(
                f"For stream {topic_stream.id} - No specific system prompt constructed; PerplexityAPI will use its internal default."
            )
        ...
        if (
            ...
            or ("no new information" in content.lower() and len(content) < 100)
        ):
            if (
                prev_summaries_concatenated_content
            ):  # Only say "no new info" if there was context
                content = "No new information is available since the last update."
            logger.warning(
                f"Received empty or 'no new info' content from API for stream {topic_stream.id}"
            )
        ...
        logger.debug(
            f"Created summary ID {summary.id} for topic stream {topic_stream.id}"
        )
        return summary

    except Exception as e:
        logger.error(
            f"Error in perform_search_and_create_summary for stream ID {topic_stream.id if topic_stream else 'Unknown'}: {str(e)}",
            exc_info=True,
        )
...
async def login(
    ...
    try:
        ...
        logger.error(
            f"Error creating access token for user {user.email}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def create_topic_stream(
    ...
    try:
        ...
        model_val = (
            ...
            else ModelType(topic_stream.model_type)
        )

        context_level_value = (
            topic_stream.context_history_level
            if topic_stream.context_history_level
            else ContextHistoryLevel.LAST_ONE.value
        )
        context_hist_level_enum = ContextHistoryLevel(context_level_value)

        logger.debug(
            f"Converted enums - freq: {update_freq}, detail: {detail_lvl}, model: {model_val}, context: {context_hist_level_enum})"
        )
        ...
        if (
            scheduler_instance and db_topic_stream.auto_update_enabled
            ...
            logger.debug(
                f"Scheduled topic stream updates for new stream ID: {db_topic_stream.id} because auto-update is enabled."
            )
        elif not db_topic_stream.auto_update_enabled:
            logger.debug(
                f"New stream ID: {db_topic_stream.id} created with auto-update disabled. Not scheduling."
            )
        else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error creating topic stream: {str(e)}"
        )
...
async def get_topic_streams(
    ...
    try:
        logger.debug(
            f"Fetching topic streams for user ID: {current_user.id}, email: {current_user.email}"
        )

        # Fetch topic streams and their associated summaries in one query for efficiency
        streams = (
            db.query(TopicStream)
            .options(joinedload(TopicStream.summaries))
            .filter(TopicStream.user_id == current_user.id)
            .all()
        )
        ...
        for stream in streams:
            ...
            stream_response_data = {
                ...
                "query": stream.query,
                "update_frequency": (
                    stream.update_frequency.value
                    if stream.update_frequency
                    else UpdateFrequency.DAILY.value
                ),
                "detail_level": (
                    stream.detail_level.value
                    if stream.detail_level
                    else DetailLevel.DETAILED.value
                ),
                "model_type": (
                    stream.model_type.value
                    if stream.model_type
                    else ModelType.SONAR_REASONING.value
                ),
                "recency_filter": (
                    stream.recency_filter if stream.recency_filter is not None else "1d"
                ),  # model default
                "last_updated": (
                ...
                "temperature": (
                    stream.temperature if stream.temperature is not None else 0.7
                ),  # model default
                "context_history_level": (
                    stream.context_history_level.value
                    if stream.context_history_level
                    else ContextHistoryLevel.LAST_ONE.value
                ),
                "total_stored_est_tokens": total_est_tokens,
                "auto_update_enabled": (
                    stream.auto_update_enabled
                    if stream.auto_update_enabled is not None
                    else True
                ),  # model default
            }
            ...
            logger.debug(
                f"Stream: ID={stream.id}, Query={stream.query}, Model={stream_response_data['model_type']}, Total Stored Tokens: {total_est_tokens}, Auto Update Enabled: {stream_response_data['auto_update_enabled']}"
            )

        return result
    except Exception as e:
        logger.error(
            f"Error fetching topic streams for user {current_user.id}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail=f"Error fetching topic streams: {str(e)}"
        )


@app.get(
    "/topic-streams/{topic_stream_id}/summaries/", response_model=List[SummaryResponse]
)
def get_topic_stream_summaries(
    ...
    try:
        ...
        logger.debug(
            f"Querying for topic stream ID: {topic_stream_id} and user ID: {current_user.id}"
        )
        topic_stream = (
            db.query(TopicStream)
            .filter(
        ...
        if not topic_stream:
            logger.warning(
                f"Topic stream {topic_stream_id} not found for user {current_user.id}"
            )
        ...
        summaries_db = (
            db.query(Summary)
            .filter(Summary.topic_stream_id == topic_stream_id)
            .order_by(Summary.created_at.desc())
            .all()
        )
        logger.debug(
            f"Found {len(summaries_db)} summaries for topic stream {topic_stream_id}"
        )
        ...
        for summary in summaries_db:
            ...
            if summary.sources:  # Check if sources string is not None or empty
                try:
                    ...
                    sources_str = (
                        summary.sources
                        if isinstance(summary.sources, str)
                        else json.dumps(summary.sources)
                    )
                    ...
                    if not isinstance(parsed_sources, list):
                        logger.warning(
                            f"Sources for summary {summary.id} is not a list, resetting to empty list"
                        )
                        parsed_sources = []

                except json.JSONDecodeError:
                    logger.warning(
                        f"Failed to decode sources JSON for summary {summary.id}: {summary.sources}"
                    )
                    ...
                    logger.error(
                        f"Unexpected error parsing sources for summary {summary.id}: {str(source_parse_error)}",
                        exc_info=True,
                    )
                    parsed_sources = []
        ...
        raise HTTPException(
            status_code=500,
            detail=f"An unexpected error occurred while fetching summaries: {str(e)}",
        )
...
def delete_topic_stream(
    ...
    topic_stream = (
        db.query(TopicStream)
        .filter(
            TopicStream.id == topic_stream_id, TopicStream.user_id == current_user.id
        )
        .first()
    )
    ...
    if not topic_stream:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Topic stream not found"
        )
    ...
    if scheduler:  # Check if the global scheduler is initialized
        ...
        logger.warning(
            f"Scheduler not available, could not remove job for stream ID: {topic_stream.id}"
        )
...
async def update_topic_stream_now(
    ...
    try:
        logger.debug(
            f"Manual update for stream {topic_stream.id}. Override ignore all previous: {options.ignore_all_previous_summaries_override}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating topic stream: {str(e)}"
        )
...
async def deep_dive(
    ...
    if not topic_stream:
        raise HTTPException(
            status_code=404, detail="Topic stream not found or not owned by user"
        )
    ...
    messages_for_perplexity.append(
        {"role": "system", "content": deep_dive_system_content}
    )
    ...
    messages_for_perplexity.append({"role": "user", "content": contextual_question})

    model_to_use = request.model if request.model else "sonar-reasoning"
    logger.debug(
        f"Deep dive for topic '{topic_stream.query}', summary ID: {request.summary_id}. Question: '{request.question}'"
    )
    logger.info(
        f"Deep Dive - Using model: {model_to_use}"
    )  # Changed to INFO for easier spotting
    ...
    try:
        ...
        logger.error(
            f"Error during Perplexity API call in deep_dive: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Error communicating with Perplexity API: {str(e)}"
        )
...
async def optimize_prompt(
    request: OptimizePromptRequest, current_user: User = Depends(get_current_user)
):
    """
...
Return ONLY the revised "Topic Query" text. Do not include any other explanatory text, preamble, or markdown formatting around the prompt itself. The output should be ready to be directly used as the new topic query."""
    ...
    try:
        logger.info(
            f"Optimizing prompt for user {current_user.id}: '{request.topic_query[:100]}...'"
        )
        ...
        optimized_query = re.sub(
            r"<think>.*?</think>", "", optimized_query, flags=re.DOTALL | re.IGNORECASE
        )

        # Remove ```think blocks specifically (not all code blocks)
        optimized_query = re.sub(
            r"```think.*?```", "", optimized_query, flags=re.DOTALL | re.IGNORECASE
        )
        ...
        return OptimizePromptResponse(optimized_query=optimized_query, model=model_used)

    except Exception as e:
        logger.error(f"Error optimizing prompt: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Error optimizing prompt: {str(e)}"
        )
...
def append_summary(
    ...
    new_summary = Summary(
        ...
        sources=json.dumps([]),
        model=(
            str(topic_stream.model_type)
            if hasattr(topic_stream, "model_type") and topic_stream.model_type
            else None
        ),
    )
    db.add(new_summary)
...
def delete_summary(
    ...
    print(
        f"DELETE SUMMARY CALLED - Stream ID: {topic_stream_id}, Summary ID: {summary_id}, User ID: {current_user.id}"
    )
    logger.info(
        f"DELETE SUMMARY CALLED - Stream ID: {topic_stream_id}, Summary ID: {summary_id}, User ID: {current_user.id}"
    )
    ...
    try:
        ...
        if not topic_stream:
            ...
            raise HTTPException(
                status_code=404, detail=f"Topic stream {topic_stream_id} not found"
            )

        # Check if the topic stream belongs to the current user
        if topic_stream.user_id != current_user.id:
            print(
                f"ERROR: Topic stream {topic_stream_id} doesn't belong to user {current_user.id}, it belongs to user {topic_stream.user_id}"
            )
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to access this topic stream",
            )
        ...
        if not raw_summary:
            ...
            raise HTTPException(
                status_code=404, detail=f"Summary {summary_id} not found in database"
            )

        # Check if the summary belongs to the topic stream
        if raw_summary.topic_stream_id != topic_stream_id:
            print(
                f"ERROR: Summary {summary_id} found but doesn't belong to topic stream {topic_stream_id}. It belongs to topic stream {raw_summary.topic_stream_id}"
            )
            raise HTTPException(
                status_code=404,
                detail=f"Summary {summary_id} not found in topic stream {topic_stream_id}",
            )
        ...
        db.execute(
            text(f"DELETE FROM summaries WHERE id = :summary_id_param"),
            {"summary_id_param": summary_id},
        )
        db.commit()
...
async def update_topic_stream(
    ...
    try:
        ...
        logger.debug(
            f"Updating topic stream {topic_stream_id} with data: {topic_stream_data}"
        )
        ...
        for field, value in topic_stream_data.dict(exclude_unset=True).items():
            if field == "update_frequency":
                ...
                setattr(
                    db_topic_stream,
                    field,
                    ModelType.R1_1776 if value == "r1-1776" else ModelType(value),
                )
                ...
                logger.info(
                    f"Updating stream {db_topic_stream.id} auto_update_enabled to: {bool_value}"
                )
            else:
        ...
        if scheduler_instance:
            if db_topic_stream.auto_update_enabled:
                logger.info(
                    f"Stream {db_topic_stream.id} updated with auto-update enabled. Re-scheduling."
                )
                scheduler_instance.schedule_topic_stream(db_topic_stream)
            else:
                logger.info(
                    f"Stream {db_topic_stream.id} updated with auto-update disabled. Removing from schedule."
                )
                scheduler_instance.remove_topic_stream(db_topic_stream.id)
        else:
            logger.error(
                "Scheduler not available, cannot interact with schedule during update."
            )
...
async def mock_update_user_preferences(
    ...
    current_user: User = Depends(
        get_current_user
    ),  # Still use current_user to ensure endpoint is protected
    # db: Session = Depends(get_db) # No db interaction for actual User model update
):
    logger.info(
        f"MOCK SAVE: User {current_user.email} attempting to save preferences: Theme={preferences.preferred_theme}, Mode={preferences.preferred_mode}"
    )
