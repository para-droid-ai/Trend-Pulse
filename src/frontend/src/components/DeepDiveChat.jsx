...
const DeepDiveChat = ({ topicStreamId, summaryId, topic, onAppend }) => {
  ...
  const handleKeyDown = (e) => {
    if (e.key === "Enter" && !e.shiftKey && !e.ctrlKey) {
      // Regular Enter without Shift or Ctrl
  ...
  const handleSaveToStream = async (message) => {
    ...
    try {
      const newSummary = await topicStreamAPI.appendSummary(
        topicStreamId,
        message.content,
      );
      if (onAppend) {
      ...
      >
        ...
        <div className="space-y-3">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`p-3 rounded-lg ${message.type === "user" ? "bg-muted ml-6" : "bg-card mr-6"} shadow-sm`}
            >
              ...
              {message.type === "user" ? (
                ...
                <div className="text-sm">
                  ...
                  <div
                    className={`dark:prose-invert overflow-hidden ${!message.isExpanded ? "line-clamp-10" : ""}`}
                  >
                    <MarkdownRenderer
          ...
          {error && (
            <div className="text-center text-destructive my-3 p-2 bg-destructive/10 rounded">
              ...
              <button
                onClick={() => setError("")}
                className="ml-2 text-xs underline text-destructive-foreground hover:text-destructive/80"
              >
                Dismiss
              </button>
            </div>
      ...
      <div className="p-3 border-t border-border bg-card">
        <form onSubmit={handleSubmit} className="space-y-2">
          ...
          <div className="flex items-center mt-2">
            ...
            />
            <label
              htmlFor="with-context-checkbox"
              className="text-xs text-muted-foreground select-none"
            >
              Send previous chat context
              <span
                className="ml-1 text-muted-foreground"
                title="If unchecked, only your current question will be sent to the model. Previous chat history will be omitted."
              >
                (?)
              </span>
            </label>
