...
function CitationNumber({ number, source, onClick }) {
  ...
  return (
    <button
      ...
      title={
        source
          ? `View source: ${typeof source === "string" ? source : source.url || source.name || "Source"}`
          : `Source ${number}`
      }
      style={{
        fontSize: "10px",
...
export default function MarkdownRenderer({ content, sources = [] }) {
  ...
  return (
    <div className="prose prose-sm sm:prose lg:prose-lg xl:prose-xl dark:prose-invert prose-p:my-2 prose-h1:mt-3 prose-h1:mb-2 prose-h2:mt-3 prose-h2:mb-2 prose-h3:mt-2 prose-h3:mb-1 px-2">
