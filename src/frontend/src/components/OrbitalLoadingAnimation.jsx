...
const OrbitalLoadingAnimation = ({
  ...
  const OrbitalVariant = () => (
    <div className={`relative ${sizeClasses[size]}`}>
      ...
      <div
        className={`absolute top-1/2 left-1/2 ${starSizes[size]} bg-gradient-radial from-yellow-400 to-orange-500 rounded-full animate-star-pulse`}
      ...
      <div className="absolute inset-0 animate-orbit-slow">
        <div
          className={`absolute ${planetSizes[size]} bg-gradient-to-br from-blue-400 to-blue-600 rounded-full`}
      ...
      >
        <div
          className={`absolute ${planetSizes[size]} bg-gradient-to-br from-purple-400 to-purple-600 rounded-full`}
          style={{ top: "0%", left: "50%", transform: "translate(-50%, -50%)" }}
        ></div>
      </div>
    </div>
  ...
  const GeometricVariant = () => (
    ...
    >
      ...
      <div
        className={`${sizeClasses[size]} border-2 border-primary animate-spin`}
        style={{ animationDuration: "2s" }}
      ></div>

      {/* Inner circle that rotates opposite direction */}
      <div
        className={`absolute inset-2 border-2 border-blue-400 rounded-full animate-spin`}
  ...
  const PulseVariant = () => (
    ...
    >
      ...
      {/* Single pulsing ring */}
      <div
        className={`absolute inset-0 border-2 border-primary/50 rounded-full animate-ping`}
      ></div>
    </div>
  );
  ...
  const WaveVariant = () => (
    ...
    >
      {/* Central point */}
      <div
        className={`${starSizes[size]} bg-gradient-to-r from-blue-400 to-purple-500 rounded-full animate-pulse`}
      ></div>
