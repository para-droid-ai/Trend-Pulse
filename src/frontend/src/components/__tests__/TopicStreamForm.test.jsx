...
describe("TopicStreamForm", () => {
  ...
  const fillForm = async (user, query = "Test query") => {
    ...
    await user.selectOptions(
      screen.getByTestId("update-frequency-select"),
      "daily",
    );
    await user.selectOptions(
      screen.getByTestId("detail-level-select"),
      "detailed",
    );
    await user.selectOptions(
      screen.getByTestId("model-type-select"),
      "sonar-reasoning",
    );
  ...
  test("shows error for short query", async () => {
    ...
    expect(
      screen.getByText("Query must be at least 3 characters"),
    ).toBeInTheDocument();
    expect(mockOnSubmit).not.toHaveBeenCalled();
  });
  ...
  test("shows error for long query", async () => {
    ...
    expect(
      screen.getByText("Query must be less than 100 characters"),
    ).toBeInTheDocument();
    expect(mockOnSubmit).not.toHaveBeenCalled();
  });
