...
export const ThemeProvider = ({ children, userId }) => {
  ...
  useEffect(() => {
    ...
    const effectiveIsDark =
      currentTheme.endsWith("-dark") || themes[currentTheme]?.isDarkOnly;
    if (effectiveIsDark) {
  ...
  useEffect(() => {
    ...
    if (isDarkMode) {
      if (themes[`${baseThemeName}-dark`]) {
        if (currentTheme !== `${baseThemeName}-dark`)
          setCurrentThemeInternal(`${baseThemeName}-dark`);
      ...
      if (currentThemeDetails?.isDarkOnly) {
        if (currentTheme !== "amber-minimal")
          setCurrentThemeInternal("amber-minimal");
        if (document.documentElement.classList.contains("dark"))
          document.documentElement.classList.remove("dark");
      } else {
        if (currentTheme !== baseThemeName)
          setCurrentThemeInternal(baseThemeName);
        if (document.documentElement.classList.contains("dark"))
          document.documentElement.classList.remove("dark");
      }
    }
