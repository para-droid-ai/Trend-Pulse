...
const Dashboard = () => {
  ...
  const [viewMode, setViewMode] = useState(
    () => localStorage.getItem("trendpulse-view-mode") || "grid",
  );
  ...
  const fetchTopicStreams = useCallback(async () => {
    ...
    try {
      ...
      const validStreams = orderedStreams.filter(
        (stream) => stream && stream.id !== undefined && stream.id !== null,
      );
      ...
      if (savedOrder) {
        try {
          ...
          const savedOrderIds = Object.keys(orderMap).map((id) =>
            parseInt(id, 10),
          );
          // Check if any ID in localStorage order is NOT in the valid streams list
          const hasStaleIds = savedOrderIds.some(
            (id) => !validStreamIds.has(id),
          );
          ...
          if (hasStaleIds) {
            ...
            const reOrderedStreams = getOrderedStreams(
              data,
              "last_updated",
              "desc",
            );
  ...
  const handleCreateStream = useCallback(
    async (newStream) => {
      ...
      const updateMessage = () => {
        ...
        currentMessageIndex = Math.floor(
      ...
      try {
        ...
        isCompleted = true;
        setCreationMessage(
          "Success! Stream created and first summary generated.",
        );
  ...
  const handleDeleteStream = useCallback(
    async (id) => {
      ...
      try {
        ...
        const updatedStreams = topicStreams.filter(
          (stream) => stream.id !== id,
        );
        ...
        if (selectedStream?.id === id) {
          setSelectedStream(
            updatedStreams.length > 0 ? updatedStreams[0] : null,
          );
        }

        // Update summaries by removing ones from the deleted stream
        setSummaries((prev) =>
          prev.filter((summary) => summary.streamId !== id),
        );
      } catch (err) {
        setError(
          `Failed to delete topic stream: ${err.response?.data?.detail || "Unknown error"}`,
        );
      }
    },
    ...
    async (id) => {
      try {
        ...
        setTopicStreams((prevStreams) => {
          ...
          const sortedStreams = getOrderedStreams(
            streamsCopy,
            "last_updated",
            "desc",
          );
  ...
  const handleDrop = (e, targetStreamId) => {
    ...
    if (draggedStreamId && dragInsertionIndex !== null) {
      ...
      const streams = getOrderedStreams(topicStreams, "manual", "asc");
      const draggedStreamIndex = streams.findIndex(
        (stream) => stream.id === draggedStreamId,
      );
  ...
  const renderSummaryItem = (summary) => {
    return (
      <div
        key={summary.id}
        className="p-3 mb-4 border rounded-lg shadow-sm bg-card border-border"
      >
        {/* Summary Header */}
        <div className="border-b border-border sticky top-[69px] bg-card z-10 pb-3">
          <div className="flex items-start justify-between mb-2">
            <h3
              className="text-lg font-semibold text-foreground line-clamp-2 max-w-[80%] Camino text-ellipsis overflow-hidden"
              title={summary.streamQuery}
            >
              {summary.streamQuery}
            </h3>
            ...
            <div className="flex flex-col items-end flex-shrink-0">
              ...
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-muted text-black dark:text-white font-medium">
                {summary.created_at
                  ? format(parseISO(summary.created_at), "MMM d, yyyy h:mm a")
                  : ""}
              </span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => {
                const stream = topicStreams.find(
                  (s) => s.id === summary.streamId,
                );
            ...
            <button
              onClick={() => {
                const streamIndex = topicStreams.findIndex(
                  (s) => s.id === summary.streamId,
                );
                if (streamIndex >= 0) {
                  changeViewMode("list");
            ...
            <div className="flex flex-wrap gap-1">
              {summary.sources.map((source, index) => (
                <a
                  ...
                  title={
                    typeof source === "string" ? source : source.url || source
                  }
                >
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex flex-col flex-1 min-h-screen overflow-x-hidden">
            ...
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  title={
                    isMacOs ? "Toggle Sidebar (⌘B)" : "Toggle Sidebar (Ctrl+B)"
                  }
                >
                  <svg
                    className="w-5 h-5"
              ...
              <div className="flex items-center space-x-3">
                ...
                <ThemeSelector
                  ...
                  onToggleRequest={() =>
                    setShowThemeSelectorModal((prev) => !prev)
                  }
                />
        ...
        {/* Main Content */}
        <main className="flex-1 px-4 py-8 overflow-x-hidden overflow-y-auto sm:px-6 lg:px-8">
                    ...
                    <div className="space-y-4">
                      {loadingSummaries ? (
                        [1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className="animate-pulse bg-card rounded-xl p-4 border min-h-[200px]"
                          >
                  ...
                  {topicStreams.length === 0 ? (
                    <div className="py-16 text-center">
                      ...
                      <h3 className="mb-2 text-xl font-semibold text-foreground">
                        No Topic Streams Yet
                      </h3>
                      <p className="mb-6 text-muted-foreground">
                        Create your first topic stream to start tracking trends
                      </p>
                      <button
                    ...
                    <div className="space-y-6">
                      {viewMode === "grid" && (
                        ...
                        >
                          {getOrderedStreams(topicStreams, "manual", "asc").map(
                            (stream) => (
                              <div
                                key={stream.id}
                                ref={(el) =>
                                  (streamRefs.current[stream.id] = el)
                                }
                        ...
                        <div className={"space-y-6"}>
                          {selectedStream ? (
                            <div
                              ...
                              ref={(el) =>
                                (streamRefs.current[selectedStream.id] = el)
                              }
                            ...
                            >
                              <TopicStreamWidget
                                ...
                                isGridView={false}
                                onDragStart={(e) =>
                                  handleDragStart(e, selectedStream.id)
                                }
                                onDragEnd={handleDragEnd}
                                onDragOver={(e) =>
                                  handleDragOver(e, selectedStream.id)
                                }
                                onDrop={(e) => handleDrop(e, selectedStream.id)}
                                isDraggedOver={
                                  dragOverStreamId === selectedStream.id
                                }
                                isDragging={
                                  draggedStreamId === selectedStream.id
                                }
                                isNewlyCreated={
                                  newlyCreatedStreamId === selectedStream.id
                                }
                                isSelected={true}
                              />
                            </div>
                            ...
                            <div className="py-16 text-center text-muted-foreground">
                              ...
                              Select a stream from the sidebar to view its
                              content.
                            </div>
                          )}
                        </div>
